    ...
    <Card className="shadow-md">
      ...
      <CardContent className="p-6 bg-white">
        <div className="text-center py-8">
          <FileImage className="mx-auto h-12 w-12 text-black" />
...
const BookImagesCard = ({ document_id, subject_id }: ImagesCardProps) => {
  const {
    data: images,
    error,
    isLoading,
          ...
          {images.map((image) => (
            ...
            >
              <div className="flex items-start space-x-4">
                ...
                <Link
                  href={`/front-pages/books/book-files?doc_id=${document_id}&file_id=${
                    image.id
                  }&subject_id=${subject_id}&return_to=${encodeURIComponent(
                    `/front-pages/books?subject_id=${subject_id}`
                  )}`}
                >
