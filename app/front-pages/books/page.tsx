...
import BookDocumentsTable from "@/app/front-pages/books/components/BookDocumentTable"
...
const Books = () => {
  ...
  useEffect(() => {
    if (subjectId) {
      ...
      form.setValue("document_type", "book")
      setFilterParams({
  ...
  const handleBack = () => {
    if (selectedSubject?.parent_id) {
      // From `/front-pages/books?subject_id=X` → go to `/front-pages?subject_id=X`
          ...
          {selectedSubject ? (
            <div className="max-w-7xl mx-auto space-y-6">
              <DocumentsTableWrapper ref={documentsTableRef}>
                <BookDocumentsTable filterParams={filterParams} />
