...
export default function ManuscriptImageDisplay() {
  ...
  const subject_id = Number(searchParams.get("subject_id")) // <-- Add this line
  ...
  const handleBack = () => {
    if (returnTo) {
      router.push(
        `/front-pages/manuscripts/${document_id}?from=${encodeURIComponent(
          `/front-pages/manuscripts?subject_id=${subject_id}&tab=images`
        )}&tab=images`
      )
    } else {
