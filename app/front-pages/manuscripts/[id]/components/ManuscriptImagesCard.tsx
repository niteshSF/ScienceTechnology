...
const ManuscriptImagesCard = ({ document_id, subject_id }: ImagesCardProps) => {
  const {
    data: images,
    error,
    isLoading,
          ...
          {images.map((image) => (
            ...
            >
              <div className="flex items-start space-x-4">
                ...
                <Link
                  href={`/front-pages/manuscripts/manuscript-files?doc_id=${document_id}&file_id=${
                    image.id
                  }&subject_id=${subject_id}&return_to=${encodeURIComponent(
                    `/front-pages/manuscripts?subject_id=${subject_id}`
                  )}`}
                >
...
export default ManuscriptImagesCard
