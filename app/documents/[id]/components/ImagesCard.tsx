...
const ImagesCard = ({ document_id }: ImagesCardProps) => {
  const {
    data: images,
    error,
    isLoading,
          ...
          {images.map((image) => (
            ...
            >
              <div className="flex items-start space-x-4">
                ...
                <Link
                  // href={`/files/?doc_id=${document_id}&file_id=${image.id}`}
                  // passHref

                  href={`/files?doc_id=${document_id}&file_id=${
                    image.id
                  }&return_to=${encodeURIComponent(
                    `/documents/${document_id}?tab=images`
                  )}`}
